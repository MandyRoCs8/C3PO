...
def process_email(message, excel_file, doc):
    # Load Excel data
...
if len(emails) == 0:
    ...
    for selected_email_index in selected_email_indices:
        ...
        excel_file = ''  # define excel_file here
        for attachment in attachments:
            if attachment.FileName.endswith('.xlsx'):
                attachment.SaveAsFile(os.path.join(os.getcwd(), attachment.FileName))
                excel_file = os.path.join(os.getcwd(), attachment.FileName)
        ...
        if not excel_file:
            ...
            process_email(message, excel_file, doc)  # pass message and excel_file to process_email()
